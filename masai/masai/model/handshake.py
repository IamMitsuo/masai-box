...
from masai.tools.tshark import Tshark
...
class Handshake(object):
    ...
    def divine_bssid_and_essid(self):
        ...
        if not self.essid and not self.bssid:
            ...
            print('{!} {O}Warning{W}: {O}Arbitrarily selected ' +
            ...
            for (bssid, essid) in pairs:
                if self.essid == essid:
                    print('{+} Discovered bssid {C}%s{W}' % bssid)
                    self.bssid = bssid
                    break
            ...
            for (bssid, essid) in pairs:
                if self.bssid.lower() == bssid.lower():
                    print('{+} Discovered essid "{C}%s{W}"' % essid)
                    self.essid = essid
                    break
    ...
    def print_pairs(pairs, capfile, tool=None):
        ...
        if len(pairs) == 0:
            print('{!} %s.cap file {R}does not{O} contain a valid handshake{W}' % (tool_str))
            return

        for (bssid, essid) in pairs:
            out_str = '{+} %s.cap file {G}contains a valid handshake{W} for' % tool_str
            if bssid and essid:
                print('%s {G}%s{W} ({G}%s{W})' % (out_str, bssid, essid))
            elif bssid:
                print('%s {G}%s{W}' % (out_str, bssid))
            elif essid:
                print('%s ({G}%s{W})' % (out_str, essid))
    ...
    def check():
        ...
        if Configuration.check_handshake == '<all>':
            print('{+} checking all handshakes in {G}"./hs"{W} directory\n')
            try:
                capfiles = [os.path.join('hs', x) for x in os.listdir('hs') if x.endswith('.cap')]
            except OSError as e:
                capfiles = []
            if len(capfiles) == 0:
                print('{!} {R}no .cap files found in {O}"./hs"{W}\n')
        else:
            capfiles = [Configuration.check_handshake]

        for capfile in capfiles:
            print('{+} checking for handshake in .cap file {C}%s{W}' % capfile)
            if not os.path.exists(capfile):
                print('{!} {O}.cap file {C}%s{O} not found{W}' % capfile)
                return
...
if __name__ == '__main__':
    ...
    hs = Handshake('./masai/tests/files/handshake_has_1234.cap', bssid='18:d6:c7:6d:6b:18', essid='YZWifi')
    hs.analyze()
    print('has_hanshake() =', hs.has_handshake())

    print('\nWith BSSID, but no ESSID specified:')
    hs = Handshake('./masai/tests/files/handshake_has_1234.cap', bssid='18:d6:c7:6d:6b:18')
    hs.analyze()
    print('has_hanshake() =', hs.has_handshake())

    print('\nWith ESSID, but no BSSID specified:')
    hs = Handshake('./masai/tests/files/handshake_has_1234.cap', essid='YZWifi')
    hs.analyze()
    print('has_hanshake() =', hs.has_handshake())

    print('\nWith neither BSSID nor ESSID specified:')
    hs = Handshake('./masai/tests/files/handshake_has_1234.cap')
    try:
        hs.analyze()
        print('has_hanshake() =', hs.has_handshake())
    except Exception as e:
        print('{O}Error during Handshake.analyze(): {R}%s{W}' % e)