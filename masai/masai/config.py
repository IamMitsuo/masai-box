...
from masai.tools.macchanger import Macchanger
...
class Configuration(object):
    ...
    def initialize(cls, load_interface=True):
        ...
        cls.wep_deauth_when_slow = 30 # Secods to give wep attack to run deauth when ivs does not increase rapidly
        ...
        cls.passlist = None
        cls.userlist = None
        for plist in password_lists:
            if os.path.exists(plist):
                cls.passlist = plist
                break
        for ulist in username_lists:
    ...
    def get_monitor_mode_interface(cls):
        if cls.interface is None:
            ...
            # from masai.tools.mon0 import Mon0
            # cls.interface = Mon0.get_mon0_interface()
            # if cls.interface is None:
            #     Mon0.start()
            #     cls.interface = Mon0.get_mon0_interface()
    ...
    def load_from_arguments(cls):
        ...
        from masai.args import Arguments
    ...
    #     if args.use_bully:
    #         from masai.tools.bully import Bully
    #         if not Bully.exists():
    #             cls.use_bully = False
    #         else:
