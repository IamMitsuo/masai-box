...
from masai.model.router import Router, WPSState
from masai.tools.mon0 import Mon0
from masai.model.station import Station
...
class Airodump(Dependency):
    ...
    def __init__(self, interface=None, channel=None, encryption=None, wps=WPSState.UNKNOWN, target_bssid=None,\
    ...
    def get_targets(self, old_targets=[], apply_filter=True):
        ...
        if csv_filename is None or not os.path.exists(csv_filename):
            return self.routers  # No file found
        ...
        for old_target in self.routers:
...
if __name__ == '__main__':
    ...
    with Airodump() as airodump:
        ...
        for idx, target in enumerate(targets, start=1):
            print('   %s %s' % (str(idx).rjust(3), target.to_str()))
